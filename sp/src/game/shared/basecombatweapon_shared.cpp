...
float g_flOriginalDOFEnabled = 0.0f;
float g_flOriginalDOFOverride = 0.0f;
float g_flOriginalMaxBlurRadius = 0.0f;
float g_flOriginalFarBlurDepth = 0.0f;

void InitializeDOFSettings()
{
	ConVar* pDOFEnabled = g_pCVar->FindVar("mat_dof_enabled");
	if (pDOFEnabled)
	{
		g_flOriginalDOFEnabled = pDOFEnabled->GetFloat();
	}

	ConVar* pDOFOverride = g_pCVar->FindVar("mat_dof_override");
	if (pDOFOverride)
	{
		g_flOriginalDOFOverride = pDOFOverride->GetFloat();
	}

	ConVar* pMaxBlurRadius = g_pCVar->FindVar("mat_dof_max_blur_radius");
	if (pMaxBlurRadius)
	{
		g_flOriginalMaxBlurRadius = pMaxBlurRadius->GetFloat();
	}

	ConVar* pFarBlurDepth = g_pCVar->FindVar("mat_dof_far_blur_depth");
	if (pFarBlurDepth)
	{
		g_flOriginalFarBlurDepth = pFarBlurDepth->GetFloat();
	}
}
...
void EnableDOF()
{
	InitializeDOFSettings();

	float transitionDuration = 1.0f;    // Transition duration in seconds
	float initialFarBlurDepth = 1000.0f; // Initial value
	float targetFarBlurDepth = 500.0f;   // Target value

	ConVar* pDOFEnabled = g_pCVar->FindVar("mat_dof_enabled");
	if (pDOFEnabled)
	{
		pDOFEnabled->SetValue(1);
	}

	ConVar* pDOFOverride = g_pCVar->FindVar("mat_dof_override");
	if (pDOFOverride)
	{
		pDOFOverride->SetValue(1);
	}

	ConVar* pMaxBlurRadius = g_pCVar->FindVar("mat_dof_max_blur_radius");
	if (pMaxBlurRadius)
	{
		pMaxBlurRadius->SetValue(10);
	}

	// Start the transition
	float g_flTransitionStartTime = gpGlobals->curtime; // Declare and define g_flTransitionStartTime here

	// Calculate the time elapsed since the transition started
	float elapsedTime = gpGlobals->curtime - g_flTransitionStartTime;

	// Calculate the interpolated value for mat_dof_far_blur_depth
	float currentFarBlurDepth = Lerp(
		elapsedTime / transitionDuration,
		initialFarBlurDepth,
		targetFarBlurDepth
		);

	// Set the calculated value for mat_dof_far_blur_depth
	ConVar* pFarBlurDepth = g_pCVar->FindVar("mat_dof_far_blur_depth");
	if (pFarBlurDepth)
	{
		pFarBlurDepth->SetValue(currentFarBlurDepth);
	}
}
...
void DisableDOF()
{
	...
	// Revert the convars to original values
	ConVar* pDOFEnabled = g_pCVar->FindVar("mat_dof_enabled");
	if (pDOFEnabled)
	{
		pDOFEnabled->SetValue(g_flOriginalDOFEnabled);
	}

	ConVar* pDOFOverride = g_pCVar->FindVar("mat_dof_override");
	if (pDOFOverride)
	{
		pDOFOverride->SetValue(g_flOriginalDOFOverride);
	}

	ConVar* pMaxBlurRadius = g_pCVar->FindVar("mat_dof_max_blur_radius");
	if (pMaxBlurRadius)
	{
		pMaxBlurRadius->SetValue(g_flOriginalMaxBlurRadius);
	}

	ConVar* pFarBlurDepth = g_pCVar->FindVar("mat_dof_far_blur_depth");
	if (pFarBlurDepth)
	{
		pFarBlurDepth->SetValue(g_flOriginalFarBlurDepth);
	}
}
