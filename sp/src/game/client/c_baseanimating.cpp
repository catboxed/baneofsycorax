...
// BOF: MUZZLE DEFAULT CONFIGS
ConVar sk_muzzle_r("sk_muzzle_r", "255");
ConVar sk_muzzle_g("sk_muzzle_g", "192");
ConVar sk_muzzle_b("sk_muzzle_b", "64");
ConVar sk_muzzle_decay("sk_muzzle_decay", "512.0f");
ConVar sk_muzzle_radius1("sk_muzzle_radius1", "245.0f");
ConVar sk_muzzle_radius2("sk_muzzle_radius2", "256.0f");
ConVar sk_muzzle_die("sk_muzzle_die", "0.05f");
...
void C_BaseAnimating::ProcessMuzzleFlashEvent()
{
	...
	if ( muzzleflash_light.GetBool() )
	{
		...
		if ( m_Attachments.Count() > 0 )
		{
			...
			dl->color.r = sk_muzzle_r.GetInt();
			dl->color.g = sk_muzzle_g.GetInt();
			dl->color.b = sk_muzzle_b.GetInt();
			dl->die = gpGlobals->curtime + sk_muzzle_die.GetFloat();
			dl->radius = random->RandomFloat(sk_muzzle_radius1.GetFloat(), sk_muzzle_radius2.GetFloat());
			dl->decay = sk_muzzle_decay.GetFloat();
