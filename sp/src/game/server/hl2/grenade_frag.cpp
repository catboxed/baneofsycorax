...
ConVar sk_frag_sprite("sk_frag_sprite", "sprites/purplelaser1.vmt");
ConVar sk_frag_startwidth("sk_frag_startwidth", "8.0f");
ConVar sk_frag_endwidth("sk_frag_endwidth", "1.0f");
ConVar sk_frag_lifetime("sk_frag_lifetime", "0.5f");
...
void CGrenadeFrag::CreateEffects(void)
{
	...
	m_pGlowTrail = CSpriteTrail::SpriteTrailCreate(sk_frag_sprite.GetString(), GetLocalOrigin(), false);
	...
	if (m_pGlowTrail != NULL)
	{
		...
		m_pGlowTrail->SetStartWidth(sk_frag_startwidth.GetFloat());
		m_pGlowTrail->SetEndWidth(sk_frag_endwidth.GetFloat());
		m_pGlowTrail->SetLifeTime(sk_frag_lifetime.GetFloat());
	}
}
...
void CGrenadeFrag::Precache(void)
{
	...
	PrecacheModel(sk_frag_sprite.GetString());
